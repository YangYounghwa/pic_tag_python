...
class IdentityEngine(threading.Thread):
    def __init__(self, shared_queue: Queue, logger=None,
                 sim_threshold=0.75, weak_threshold=0.6,
                 prototype_add_delta=0.05,
                 max_prototypes=3, spatial_bias=0.05,
                 spatial_window_size=10, spatial_distance_thresh=20,
                 max_age_sec=86400, max_history=20000):
        super().__init__()
        self.queue = shared_queue
        self.logger = logger


        self.strong_threshold = sim_threshold + prototype_add_delta
        self.weak_threshold = weak_threshold
        self.prototype_add_thresh = sim_threshold
        ...
        self.recent_detections = defaultdict(lambda: deque(maxlen=spatial_window_size))  # camera_id → recent detection window
        ...
        with self.lock:
            ...
            for pid, prototypes in self.prototype_db.items():
                ...
                for ts_recent, seen_pid, seen_bbox in self.recent_detections[camera_id]:
                    if seen_pid == pid and self._bbox_distance(bbox, seen_bbox) < self.spatial_distance_thresh**2:
            ...
            # No good match — create new identity
    ...
    def _update_identity(self, pid, timestamp, embedding, bbox, camera_id):
        ...
        if max_sim >= self.strong_threshold:
            # EMA update for similar prototype
            decay = 0.9
            updated = decay * prototypes[best_idx] + (1 - decay) * embedding
            prototypes[best_idx] = updated / np.linalg.norm(updated)

        elif max_sim < self.prototype_add_thresh:
            if len(prototypes) < self.max_prototypes:
                prototypes.append(embedding)
            else:
                min_idx = int(np.argmin(sims))
                prototypes[min_idx] = embedding  # optionally: skip this line if you want to preserve old views
    ...
    def _update_recent_detections(self, camera_id, timestamp, pid, bbox):
        self.recent_detections[camera_id].append((timestamp, pid, bbox))

    def _is_spatially_near(self, pid, bbox, camera_id):
        for ts_recent, seen_pid, seen_bbox in self.recent_detections[camera_id]:
            if seen_pid == pid and self._bbox_distance(bbox, seen_bbox) < self.spatial_distance_thresh**2:
                return True
        return False
    ...
    def _bbox_distance(self, b1, b2):
        ...
        return dx * dx + dy * dy  # no sqrt for speed
